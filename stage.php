...
<html lang="fr">
  <head>
    ...
    <title>Ajouter un nouvel utilisateur au serveur LDAP <?php echo '[<span class="red">'.$CONSTANTES['adresseIp'].':'.$CONSTANTES['port'].'</span>]'; ?></title> <!-- On récupère simplement l'adresse ip du serveur et le port sur lequel on se connecte pour savoir de quoi on parle. -->
		...
		if(preg_match("#^[a-zA-Z-]+$#",$_POST['nom']) AND preg_match("#^[a-zA-Z-]+$#",$_POST['prenom']))
		{
			...
			if($mdp == $confmdp )
			{
				...
				if ($ds)
				{
					...
					echo '<br />liaison avec le serveur effectué';
