function modUser(user)
{
	...
	xhr.onreadystatechange = function()
	{
		...
		if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0))
		{
			...
			if (mess == 'L\'utilisateur a été modifié mais certaines erreures persistent. Un message sera envoyé à l\'administrateur.' || mess == 'L\'utilisateur a été correctement modifié.')
			{
				...
				js.text = 'setTimeout("window.location=\'deco.php\'",1000);';
				head.appendChild(js);
	...
	if(nom.length > 0 && prenom.length > 0 && mail.length > 0)
	{
		...
		if(mdp.length > 0 && confmdp.length > 0 && mdp == confmdp)
		{
		...
		if(file.length > 0)
		{
			...
			if(file.name.toLowerCase().lastIndexOf('jpg') >= 0 ||  file.name.toLowerCase().lastIndexOf('jpeg') >= 0 || file.name.toLowerCase().lastIndexOf('png') >= 0 || file.name.toLowerCase().lastIndexOf('bmp') >= 0 || file.name.toLowerCase().lastIndexOf('gif') >= 0)
			{
				...
				if(ext)
				{
					...
					xhr.open("POST", "api/modUser.php", true);
					xhr.send(formData);
				}
				else
				{
