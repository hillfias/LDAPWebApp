function kickUser(user,group,bool)
{
	function remove(id) {
	...
	if(bool == 'true'){ deleteAdmin(user,group); setTimeout(function()
	{
		// délai de x secondes
		var yo = document.getElementById('main-panel').innerHTML;
		if(yo == '<p class="center success">L\'admin a été correctement supprimé.</p>')
		{
			kickUser(user,group, "false");
		}
	...
	else
	{
		...
		xhr.onreadystatechange = function()
		{
			...
			if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0))
			{
				...
				if (mess == 'L\'utilisateur a été correctement enlever du groupe.')
				{
		...
		xhr.open("POST", "api/kickUser.php", true);
		xhr.send(formData);
	}
